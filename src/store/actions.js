...
export const HIGHSCORES_NEW_SCORE_CHECK_START =
  "HIGHSCORES_NEW_SCORE_CHECK_START";
export const HIGHSCORES_NEW_SCORE_CHECK_END = "HIGHSCORES_NEW_SCORE_CHECK_END";
export const HIGHSCORES_NEW_SCORE_CHECK_ERROR =
  "HIGHSCORES_NEW_SCORE_CHECK_ERROR";
...
export const USER_GET_PERSONAL_BESTS_FROM_STORAGE =
  "USER_GET_PERSONAL_BESTS_FROM_STORAGE";
...
export const USER_PATCH_PERSONAL_BEST_SUCCESS =
  "USER_PATCH_PERSONAL_BEST_SUCCESS";
...
const authUser = (userData, isSignup) => {
  return (dispatch) => {
    const user = {
      ...
      url = isSignup
        ? AUTH_URL + "accounts:signUp?key="
        : AUTH_URL + "accounts:signInWithPassword?key=";
    ...
    axios
              ...
              dispatch(
                authFail(
                  error.response.data.error || {
                    message: "Posting new user data - unknown error",
                  }
                )
              );
            });
      ...
      .catch((error) => {
        dispatch(
          authFail(
            error.response.data.error || {
              message: "Logging in / Signing up - unknown error",
            }
          )
        );
      });
    ...
    axios
      ...
      .then((res) => {
        ...
        if (keysArr.length === 1) {
          ...
          dispatch(
            authFail({ message: "Getting user data from USERS failed" })
          );
        }
      })
      .catch((error) => {
        dispatch(
          authFail(
            error.response.data.error || {
              message: "Logging in - unknown error",
            }
          )
        );
      });
  ...
  return (dispatch) => {
    ...
    try {
      ...
      expirationDate = new Date(
        localStorage.getItem("slidePuzzleExpirationDate")
      );
    ...
    if (idToken && localId && expirationDate) {
      ...
      if (expirationDate > new Date()) {
        const secondsToLogoutLeft = Math.floor(
    ...
    checkIfUsernameAvailable(newUsername)
      .then(() => {
        axios
          ...
          .put(
            `users/${getState().user.usersId}/username.json?auth=${
              getState().user.idToken
            }`,
            `"${newUsername}"`
          )
          .then((res) => {
            ...
            if (usernameExistInHS) {
              ...
              axios.get("highscores.json").then((res) => {
                if (res.data) {
                  ...
                  axios
                    .patch(
                      `highscores.json?auth=${getState().user.idToken}`,
                      highscores
                    )
                    .then((res) => {
          ...
          .catch((error) => {
            dispatch(
              changeUsernameFail(
                error.response
                  ? error.response.data.error
                  : { message: "Changing username - unknown error" }
              )
            );
          });
      })
              ...
              .catch((error) => {
                dispatch({
                  ...
                  error: error.response
                    ? error.response.data.error
                    : { message: "Changing password - unknown error" },
                });
              });
          })
          .catch((error) => {
...
export const sendPasswordResetEmail = (email, messages) => {
  return (dispatch) => {
    ...
    axios
      .post(AUTH_URL + "accounts:sendOobCode?key=" + KEY, data)
      .then((res) => {
        data = { email: res.data.email };
        dispatch({ type: SEND_PASSWORD_RESET_EMAIL_SUCCESS });
        dispatch({ 
					type: MESSAGE_SHOW,
					message: messages.emailWasSent,
				});
      })
      .catch((error) => {
				dispatch({ 
					type: MESSAGE_SHOW,
					message: messages.errorOccured + error.response.data.error.message,
				});
				dispatch({
          type: SEND_PASSWORD_RESET_EMAIL_FAIL,
          error: error.response
            ? error.response.data.error
            : { message: "Reset password - unknown error" },
				});
    ...
    return axios
      ...
      .catch((error) => {
        dispatch(
          authFail(
            error.response.data.error || {
              message: "Getting highscores - unknown error",
            }
          )
        );
      });
...
export const userNewPersonalBest = (gameType, time) => {
  return (dispatch, getState, type = gameType, tim = time) => {
    ...
    if (!getState().user.anonymous) {
      ...
      axios
        .patch(
          `users/${getState().user.usersId}/personalBests.json?auth=${
            getState().user.idToken
          }`,
          { [type]: tim }
        )
...
export const highscoresNewScoreCheck = (gameType, score) => {
  return (dispatch, getState) => {
    ...
    axios
      .get("highscores.json")
      .then((res) => {
        const highscores = res.data[gameType],
          newHighscore = {
            score: score,
        ...
        if (highscores) {
          ...
          if (rank !== null) {
            updatedHighscores = highscores
              .slice(0, rank)
              .concat(newHighscore, highscores.slice(rank));
        ...
        if (updatedHighscores) {
          ...
          axios
            .patch(`highscores.json?auth=${getState().user.idToken}`, {
              [gameType]: updatedHighscores,
            })
            .then((res) => {
      ...
      .catch((error) => {
        dispatch({
          ...
          error: error.response.data.error || {
            message: "Unable to load highscores due to unknown error",
          },
        });
      });
  };
